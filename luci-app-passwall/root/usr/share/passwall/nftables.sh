...
RULE_LAST_INDEX() {
	[ $# -ge 3 ] || {
		echolog "Incorrect index listing method (nftables), execution terminated!"
		return 1
	}
	local table_name="${1}"; shift
	local chain_name="${1}"; shift
...
load_acl() {
			...
			for i in $(cat $_acl_list); do
				...
				[ "$tcp_no_redir_ports" != "disable" ] && {
					if ! has_1_65535 "$tcp_no_redir_ports"; then
						...
						echolog "     - ${msg}Do not proxy TCP port[${tcp_no_redir_ports}]"
					else
						#结束时会return，无需加多余的规则。
						unset tcp_port
				...
				[ "$udp_no_redir_ports" != "disable" ] && {
					if ! has_1_65535 "$udp_no_redir_ports"; then
						...
						echolog "     - ${msg}Do not proxy UDP port[${udp_no_redir_ports}]"
					else
						#结束时会return，无需加多余的规则。
						unset udp_port
				...
				[ -n "$tcp_port" ] && {
					if [ -n "${tcp_proxy_mode}" ]; then
						msg2="${msg}Using TCP nodes[$tcp_node_remark]"
						if [ -n "${is_tproxy}" ]; then
				...
				[ -n "$udp_port" ] && {
					if [ -n "${udp_proxy_mode}" ]; then
						msg2="${msg}Using UDP nodes[$udp_node_remark]"
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && [ "$CLIENT_PROXY" == 1 ] && {
		...
		if [ -n "${TCP_PROXY_MODE}" ]; then
			[ -n "$TCP_NODE" ] && {
				if is_socks_wrap "$TCP_NODE"; then
					msg2="${msg}Using TCP nodes[Socks configuration($(config_n_get ${TCP_NODE#Socks_} port) port)]"
				else
					msg2="${msg}Using TCP nodes[$(config_n_get $TCP_NODE remarks)]"
				fi
				if [ -n "${is_tproxy}" ]; then
		...
		if [ -n "${UDP_PROXY_MODE}" ]; then
			[ -n "$UDP_NODE" -o "$TCP_UDP" = "1" ] && {
				if is_socks_wrap "$UDP_NODE"; then
					msg2="${msg}Using UDP nodes[Socks configuration($(config_n_get ${UDP_NODE#Socks_} port) port)](TPROXY:${UDP_REDIR_PORT})"
				else
					msg2="${msg}Using UDP nodes[$(config_n_get $UDP_NODE remarks)](TPROXY:${UDP_REDIR_PORT})"
				fi
...
filter_haproxy() {
	...
	echolog "  - [$?]Add the load balancer node to nftset[$NFTSET_VPS]Direct connection completed"
}
...
add_firewall_rule() {
	...
	[ "$USE_DIRECT_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?]Parsing and adding GeoIP to the [Direct Connect List] to NFTSET complete."
			fi
		}
	}
	...
	[ "$USE_PROXY_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?]Parsing and adding GeoIP to the [agent list] to NFTSET complete."
			fi
		}
	}
	...
	[ "$USE_BLOCK_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?]Parsing and adding GeoIP to the [block list] to NFTSET complete."
			fi
		}
	}
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		if [ -n "$GEOIP_CODE" ]; then
			...
			echolog "  - [$?]Parsing and adding the [split node] GeoIP to NFTSET is complete."
		fi
	}
	...
	[ -n "$lan_ifname" ] && {
		...
		#echolog "This machineIPv4网段互访直连：${lan_ip}"
		#echolog "This machineIPv6网段互访直连：${lan_ip6}"
	...
	[ -n "$IPT_APPEND_DNS" ] && {
		...
		for local_dns in $(echo $IPT_APPEND_DNS | tr ',' ' '); do
			...
			if echo "$dns_address" | grep -q -v ':'; then
				...
				echolog "  - [$?]Add direct DNS to nftables：${dns_address}:${dns_port:-53}"
			else
		...
		if [ -n "$TCP_NODE" ]; then
			_proxy_tcp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					...
					[ $? -eq 0 ] && {
						echolog "  - The upstream DNS server ${2} is already on the direct access list, so TCP/${3} port access to this server is not forced to be forwarded to the TCP proxy."
						return 0
					}
					if [ -z "${is_tproxy}" ]; then
						...
						nft insert rule $NFTABLE_NAME PSW_MANGLE ip protocol tcp iif lo tcp dport ${3} ip daddr ${2} $(REDIRECT $TCP_REDIR_PORT TPROXY4) comment \"This machine\"
					fi
					echolog "  - [$?]Add the upstream DNS server ${2}:${3} to the TCP forwarding chain proxied by the router itself."
				else
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo tcp dport ${3} ip6 daddr ${2} $(REDIRECT $TCP_REDIR_PORT TPROXY6) comment \"This machine\""
			...
			[ -n "${LOCALHOST_TCP_PROXY_MODE}" ] && {
				...
				[ -n "${is_tproxy}" ] && nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol tcp iif lo $(REDIRECT $TCP_REDIR_PORT TPROXY4) comment \"This machine\""
			}
			[ -n "${is_tproxy}" ] && nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol tcp iif lo counter return comment \"This machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				[ -n "${LOCALHOST_TCP_PROXY_MODE}" ] && {
					...
					nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo $(REDIRECT $TCP_REDIR_PORT TPROXY) comment \"This machine\""
				}
				nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo counter return comment \"This machine\""
			}
		fi
		...
		if [ -n "$UDP_NODE" -o "$TCP_UDP" = "1" ]; then
			_proxy_udp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					nft "get element $NFTABLE_NAME $NFTSET_LAN {${2}}" &>/dev/null
					[ $? == 0 ] && {
						echolog "  - The upstream DNS server ${2} is already on the direct access list, so we will not force the forwarding of access to this server's UDP/${3} port to the UDP proxy."
						return 0
					}
					nft "insert rule $NFTABLE_NAME PSW_OUTPUT_MANGLE ip protocol udp ip daddr ${2} udp dport ${3} counter jump PSW_RULE"
					nft "insert rule $NFTABLE_NAME PSW_MANGLE ip protocol udp iif lo ip daddr ${2} $(REDIRECT $UDP_REDIR_PORT TPROXY4) comment \"This machine\""
					echolog "  - [$?]Add the upstream DNS server ${2}:${3} to the UDP forwarding chain proxied by the router itself."
				else
					nft "get element $NFTABLE_NAME $NFTSET_LAN6 {${2}}" &>/dev/null
					[ $? == 0 ] && {
						echolog "  - The upstream DNS server ${2} is already on the direct access list, so we will not force the forwarding of access to this server's UDP/${3} port to the UDP proxy."
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto udp iif lo ip6 daddr ${2} $(REDIRECT $UDP_REDIR_PORT TPROXY6) comment \"This machine\""
			...
			[ -n "${LOCALHOST_UDP_PROXY_MODE}" ] && {
				...
				nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol udp iif lo $(REDIRECT $UDP_REDIR_PORT TPROXY4) comment \"This machine\""
			}
			nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol udp iif lo counter return comment \"This machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				[ -n "${LOCALHOST_UDP_PROXY_MODE}" ] && {
					...
					nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto udp iif lo $(REDIRECT $UDP_REDIR_PORT TPROXY) comment \"This machine\""
				}
				nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto udp iif lo counter return comment \"This machine\""
			}
		fi
...
del_firewall_rule() {
	...
	echolog "The deletion of the nftables rule is complete."
}
