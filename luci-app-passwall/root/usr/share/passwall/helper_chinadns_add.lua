...
if is_file_nonzero(file_vpslist) then
	...
	log(string.format("  - Domains in the node list (vpslist)：%s", DNS_LOCAL or "default"))
end
...
if USE_DIRECT_LIST == "1" and is_file_nonzero(file_direct_host) then
	...
	log(string.format("  - Domain whitelist：%s", DNS_LOCAL or "default"))
end
...
if USE_PROXY_LIST == "1" and is_file_nonzero(file_proxy_host) then
	...
	log(string.format("  - Blacklist of proxy domains：%s", DNS_TRUST or "default"))
end
...
if GFWLIST == "1" and is_file_nonzero(RULES_PATH .. "/gfwlist") then
	...
	log(string.format("  - Firewall Domain Name List (gfwlist)：%s", DNS_TRUST or "default"))
end
...
if CHNLIST ~= "0" and is_file_nonzero(RULES_PATH .. "/chnlist") then
	if CHNLIST == "direct" then
		...
		merge_array(config_lines, tmp_lines)
		log(string.format("  - Chinese Domain Name List (chnroute)：%s", DNS_LOCAL or "default"))
	end
	...
	if CHNLIST == "proxy" then
		...
		log(string.format("  - Chinese Domain Name List (chnroute)：%s", DNS_TRUST or "default"))
	end
end
...
if IS_SHUNT_NODE then
	...
	uci:foreach(appname, "shunt_rules", function(s)
		...
		if _node_id and _node_id ~= "_blackhole" then
			...
			if _node_id ~= "_direct" then
				log(string.format("  - Sing-Box/Xray Trimming Rules(%s)：%s", s.remarks, DNS_TRUST or "default"))
			end
		end
	end)
...
log("  - ChinaDNS-NG is now the upstream provider for Dnsmasq. If you configure the DNS process incorrectly, it will cause domain name (direct connection/proxy domain) splitting to fail！！！")
