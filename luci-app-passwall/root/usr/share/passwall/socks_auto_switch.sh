...
test_auto_switch() {
	...
	if [ "$restore_switch" = "1" ] && [ -n "$main_node" ] && [ "$now_node" != "$main_node" ]; then
		...
		[ $? -eq 0 ] && {
			check_process
			#主节点正常，切换到主节点
			echolog "Socks switching detection：Port[${socks_port}] Master node【$(config_n_get $main_node type)：[$(config_n_get $main_node remarks)]】Normal, switch to master node!"
	...
	if [ "$status" = "0" ]; then
		...
		if [ "$backup_node_num" -gt 1 ]; then
			...
			msg="Switch to$([ "$now_node" = "$main_node" ] && echo backup node || echo Next backup node)Test!"
		else
			# 只有一个后备节点时，与主节点轮询
			new_node=$([ "$now_node" = "$main_node" ] && echo "$b_nodes" || echo "$main_node")
			msg="Switch to$([ "$now_node" = "$main_node" ] && echo backup node || echo Master node)Test!"
		fi
