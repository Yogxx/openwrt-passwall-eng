	...
	for k, v in ipairs(url) do
		...
		if v ~= "geo2rule" then
			for i = 1, max_attempts do
				...
				log(string.format("%s Rule # %d failed to download (HTTP:%s)，Currently conducting the %dth attempt...", rule_name, k, tostring(http_code), i))
			end
		else
...
local function check_instance(action)
	...
	if fs.access(sub_lock) then
		log("The [Subscription] instance is running; the [Rule Update] instance has entered the queue and is waiting...\n")
	end
