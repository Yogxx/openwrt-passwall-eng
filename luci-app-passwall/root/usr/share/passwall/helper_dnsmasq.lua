...
function add_rule(var)
		...
		if USE_DIRECT_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/direct_host") then
				...
				if fwd_dns then
					...
					log(string.format("  - Domain whitelist：%s", fwd_dns or "default"))
				end
			end
		end
		...
		if USE_PROXY_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/proxy_host") then
				...
				if fwd_dns then
					...
					log(string.format("  - Blacklist of proxy domains：%s", fwd_dns or "default"))
				end
			end
		end
		...
		if CHN_LIST ~= "0" then
			if fs.access("/usr/share/passwall/rules/chnlist") then
				...
				if fwd_dns then
					...
					log(string.format("  - Chinese Domain Name List (chnroute)：%s", fwd_dns or "default"))
				end
			end
		end
	...
	if USE_CHINADNS_NG == "0" then
		log("  - PassWall depends on Dnsmasq. If you configure the DNS process incorrectly, it will cause domain name (direct connection/proxy domain) splitting to fail！！！")
	end
end
